...
def plot_lines(i, ax):
    ...
    t = np.linspace(0, 1)
    model_dummy, spline, spline_x = fit_lin(0, 100, True)
    ...
    lines_refund = pd.read_csv(f"sim_data/lines/line{i+1}.csv", index_col=0)
    ...
    ax[1, 1].plot(lines_refund.iloc[:, 0], lines_refund.iloc[:, 3], color="grey", alpha=0.1)
...
mod = fit_smooth(1, 10000)

x = np.linspace(-5, 5, 100)
t = np.linspace(0, 1, 100)
mesh = np.meshgrid(x, t)
y_plot = interaction_effect(mesh[1].flatten(), mesh[0].flatten(), marginal=True)
...
x_plot = mod[1][3].tranform_new(mesh[0].flatten(), mesh[1].flatten())
y_plot_hat = x_plot @ mod[0].weights[0][9:]
pd.DataFrame(np.column_stack([mesh[0].flatten(), mesh[1].flatten(), y_plot_hat]), columns=["x", "t", "y"]).to_csv("sim_data/lines/my_smoo1.csv")

fig = plt.figure(figsize=(6, 4))

#ax.1contour(x,t, y_plot, colors="black", levels=levels, linewidths=linewidths, linestyles="solid")
ax1 = fig.add_subplot(111, projection="3d")
ax1.plot_surface(mesh[0], mesh[1], y_plot.reshape(100, 100), cmap="autumn")

ax1.contourf(mesh[0], mesh[1], y_plot.reshape(100, 100), zdir='z', offset=y_plot.min()-1, cmap='autumn', alpha=0.7)
ax1.set_zlim(y_plot.min()-1, y_plot.max())
ax1.set_xlabel("x")
ax1.set_ylabel("t")
ax1.set_zlabel(r"$f(x, t)$")
...
df_my = np.array(pd.read_csv("sim_data/lines/my_smoo1.csv", index_col=0))
df_refund = np.array(pd.read_csv("sim_data/lines/smooth.csv", index_col=0))


ax1.plot_surface(df_my[:, 0].reshape(100, 100), df_my[:, 1].reshape(100, 100), df_my[:, 2].reshape(100, 100))
ax1.contourf(df_my[:, 0].reshape(100, 100), df_my[:, 1].reshape(100, 100), df_my[:, 2].reshape(100, 100), zdir='z', offset=y_plot.min()-1, cmap='autumn', alpha=0.7)
ax1.set_zlim(df_my[:, 2].min()-1, df_my[:, 2].max())
ax1.set_xlabel("x")
ax1.set_ylabel("t")
ax1.set_zlabel(r"$f(x, t)$")


ax2.plot_surface(df_refund[:, 0].reshape(100, 100), df_refund[:, 1].reshape(100, 100), df_refund[:, 2].reshape(100, 100), cmap="autumn")
ax2.contourf(df_refund[:, 0].reshape(100, 100), df_refund[:, 1].reshape(100, 100), df_refund[:, 2].reshape(100, 100), zdir='z', offset=y_plot.min()-1, cmap='autumn', alpha=0.7)
ax2.set_zlim(df_refund[:, 2].min()-1, df_refund[:, 2].max())
ax2.set_xlabel("x")
ax2.set_ylabel("t")
